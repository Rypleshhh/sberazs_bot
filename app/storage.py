...
def get_available_since(api_id: str, fuel_type: str, conn=None) -> Optional[str]:
    """С какого момента топливо доступно непрерывно (для тайм-лога) —
    идём по истории от новой записи к старой, пока статус 'available'.
    Если передан conn (внутри текущего скана) — используем его, чтобы
    видеть записи истории, ещё не закоммиченные в этой же транзакции."""
...
def get_fuel_since_map(api_id: str, fuels_json: Optional[str], conn=None) -> dict:
    ...
    for ftype, fdata in fuels.items():
        if fdata.get("availabilityStatus") == "available":
            since = get_available_since(api_id, ftype, conn=conn)
            if since:
                result[ftype] = since
    return result
    ...
    with closing(_connect()) as conn:
        for api_id, st in stations.items():
            ...
            if resumed and is_recent(row["last_payment_at"], max_age_minutes):
                ...
                # Полная разбивка по всем видам топлива + тайм-лог "доступно
                # с..." для остальных, не только для того, что резко
                # появилось — чтобы уведомление давало полную картину.
                since_map = get_fuel_since_map(api_id, row["fuels_json"], conn=conn)
                notify_row["fuel_since_json"] = json.dumps(since_map, ensure_ascii=False)
                updates_to_notify.append(notify_row)
                conn.execute(
                    """
                    INSERT INTO pending_notifications
                        (area, name, address, resumed_fuels_json, fuels_json, fuel_since_json,
                         operations_count, last_payment_at, tbank_json, gdebenz_json)
                    VALUES (:area, :name, :address, :resumed_fuels_json, :fuels_json, :fuel_since_json,
                            :operations_count, :last_payment_at, :tbank_json, :gdebenz_json)
                    """,
                    notify_row,
                )
