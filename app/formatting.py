import json
from datetime import datetime, timedelta, timezone

from app.storage import FUEL_LABELS, FUEL_STATUS_LABELS
from app.gdebenz_client import GDEBENZ_STATUS_LABELS

# sberazs отдаёт время с +03:00 (Москва) — приводим к этому же поясу
# всё остальное (историю сканов и т.п.), чтобы времена в одной
# карточке визуально совпадали, а не расходились на 3 часа.
_DISPLAY_TZ = timezone(timedelta(hours=3))


def format_payment_time(raw: str | None) -> str:
    """Превращает время в единый пояс отображения '20:17:21 12.07.2026'."""
    if not raw:
        return "—"
    try:
        dt = datetime.fromisoformat(raw)
    except ValueError:
        return raw
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)
    dt = dt.astimezone(_DISPLAY_TZ)
    return dt.strftime("%H:%M:%S %d.%m.%Y")


def _parse_fuels(fuels_json: str | None) -> list[dict]:
    if not fuels_json:
        return []
    try:
        return json.loads(fuels_json)
    except (TypeError, ValueError):
        return []


def _format_fuel_line(fuel: dict, since_map: dict) -> str:
    ftype = fuel.get("type", "?")
    label = FUEL_LABELS.get(ftype, ftype)
    status = fuel.get("availabilityStatus", "unknown")
    status_label = FUEL_STATUS_LABELS.get(status, status)
    line = f"  • {label}: {status_label}"
    if fuel.get("limitLiters"):
        line += f" (лимит {fuel['limitLiters']} л)"
    since = since_map.get(ftype)
    if since:
        line += f"\n    ⏱ доступно с {format_payment_time(since)}"
    return line


TBANK_STATUS_LABELS = {
    "available": "есть",
    "maybe_available": "возможно есть",
    "no_data": "нет данных",
    "not_available": "нет",
}


def _format_tbank_line(tbank_json: str | None) -> str:
    if not tbank_json:
        return ""
    try:
        t = json.loads(tbank_json)
    except (TypeError, ValueError):
        return ""
    if not t:
        return ""

    status = t.get("status", "no_data")
    label = TBANK_STATUS_LABELS.get(status, status)
    confidence = t.get("confidence")
    conf_text = f" ({round(confidence * 100)}%)" if confidence is not None else ""
    return f"\n🏦 T-Bank подтверждает: {label}{conf_text}"


def _format_gdebenz_line(gdebenz_json: str | None) -> str:
    if not gdebenz_json:
        return ""
    try:
        g = json.loads(gdebenz_json)
    except (TypeError, ValueError):
        return ""
    if not g:
        return ""

    status = g.get("status", "")
    label = GDEBENZ_STATUS_LABELS.get(status, status or "нет данных")
    return f"\n🚗 gdebenz (отметки водителей): {label}"


def format_station(st: dict) -> str:
    """Полная карточка станции — для /scan_city и /cities."""
    fuels = _parse_fuels(st.get("fuels_json"))
    try:
        since_map = json.loads(st.get("fuel_since_json") or "{}")
    except (TypeError, ValueError):
        since_map = {}

    fuels_block = "\n".join(_format_fuel_line(f, since_map) for f in fuels) if fuels else "  нет данных"
    ops = st.get("operations_count") or 0
    ops_line = f"\n👥 Заправились за последнее время: {ops}" if ops else ""

    return (
        f"⛽ <b>{st.get('name') or 'Без названия'}</b>\n"
        f"📍 {st.get('address') or '—'}\n"
        f"🛢 Топливо:\n{fuels_block}"
        f"{ops_line}\n"
        f"🕒 Последняя оплата: {format_payment_time(st.get('last_payment_at'))}"
        f"{_format_tbank_line(st.get('tbank_json'))}"
        f"{_format_gdebenz_line(st.get('gdebenz_json'))}"
    )


def format_resumption(st: dict) -> str:
    """Уведомление о том, что топливо СНОВА появилось после перерыва —
    используется для push-уведомлений подписчикам. Помимо самого факта
    возврата показывает полную разбивку по всем видам топлива станции
    с тайм-логом ('доступно с...') для тех, что уже были в наличии."""
    resumed = _parse_fuels(st.get("resumed_fuels_json"))
    resumed_types = {f.get("type") for f in resumed}

    labels = []
    for f in resumed:
        label = FUEL_LABELS.get(f.get("type"), f.get("type"))
        if f.get("limitLiters"):
            label += f" (лимит {f['limitLiters']} л)"
        labels.append(label)
    fuels_text = ", ".join(labels) if labels else "топливо"

    all_fuels = _parse_fuels(st.get("fuels_json"))
    try:
        since_map = json.loads(st.get("fuel_since_json") or "{}")
    except (TypeError, ValueError):
        since_map = {}

    fuels_block = ""
    if all_fuels:
        lines = []
        for f in all_fuels:
            marker = "🆕 " if f.get("type") in resumed_types else "  "
            lines.append(marker + _format_fuel_line(f, since_map).strip())
        fuels_block = "\n🛢 Топливо:\n" + "\n".join(f"  {l}" for l in lines)

    ops = st.get("operations_count") or 0
    ops_line = f"\n👥 Заправились за последнее время: {ops}" if ops else ""

    return (
        f"⛽ <b>{st.get('name') or 'Без названия'}</b>\n"
        f"📍 {st.get('address') or '—'}\n"
        f"🎉 Снова в наличии: {fuels_text}"
        f"{fuels_block}"
        f"{ops_line}\n"
        f"🕒 Последняя оплата: {format_payment_time(st.get('last_payment_at'))}"
        f"{_format_tbank_line(st.get('tbank_json'))}"
        f"{_format_gdebenz_line(st.get('gdebenz_json'))}"
    )


def format_station_list(stations: list[dict], header: str) -> list[str]:
    """
    Возвращает список сообщений (Telegram режет длинные сообщения,
    поэтому группируем станции пачками, а не одним гигантским текстом).
    """
    if not stations:
        return [f"{header}\n\nСвежих данных не найдено."]

    messages = []
    chunk = [header, ""]
    chunk_len = len(header)

    for st in stations:
        block = format_station(st)
        if chunk_len + len(block) > 3500:
            messages.append("\n\n".join(chunk))
            chunk = []
            chunk_len = 0
        chunk.append(block)
        chunk_len += len(block)

    if chunk:
        messages.append("\n\n".join(chunk))

    return messages
