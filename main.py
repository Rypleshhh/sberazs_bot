...
async def cmd_scan_city(message: Message):
    ...
    area = parts[1].strip()
    command_id = storage.add_command(message.chat.id, area)
    log.info("Создана команда id=%s chat_id=%s area=%s", command_id, message.chat.id, area)
    await message.answer(f"Принял запрос по «{area}», жду ответа от сервера-скрапера...")
    asyncio.create_task(_watch_command_timeout(command_id, message.chat.id, area))
...
async def cmd_unknown(message: Message):
    ...
    await message.answer(
        ...
        "/scan_city, /subscribe, /unsubscribe, /my_subscriptions"
    )
